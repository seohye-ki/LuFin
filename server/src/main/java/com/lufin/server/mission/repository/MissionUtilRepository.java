...
public interface MissionUtilRepository extends JpaRepository<Mission, Integer> {
	// (선생님) 미션 삭제
	@Modifying
	@Query(value = "DELETE FROM missions WHERE mission_id = :missionId AND class_id = :classroomId", nativeQuery = true)
	void deleteMission(Integer classroomId, Integer missionId);

	// 미션의 classId 조회
	@Query(value = "SELECT class_id FROM missions WHERE mission_id = :missionId", nativeQuery = true)
